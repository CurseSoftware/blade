#ifndef BLADE_GFX_VULKAN_UTILS_H
#define BLADE_GFX_VULKAN_UTILS_H
...
namespace blade
{
    namespace gfx
    {
        namespace vk
        {
            /// @brief Create a string out of a `VkResult` value 
            /// @param res `VkResult` to create the error string from
            /// @return `std::string` of the corresponding VkResult
            inline std::string error_string(VkResult res) 
            {
                switch (res) {
            ...
            /// @return `const char*` of the corresponding `VkResult` value
            inline const char* vkresult_string(VkResult result, bool get_extended) 
            {
            ...
            /// @brief Discriminate `VkResult` values based on success or error
            /// @param result `VkResult` to determine is successful or not
            /// @return `true` if is successful, `false` otherwise
            inline bool vkresult_is_success(VkResult result) 
            {
                // From: https://www.khronos.org/registry/vulkan/specs/1.2-extensions/man/html/VkResult.html
                switch (result) {
                        // Success Codes
            ...
            /// @brief Macro utility for assserting successful vulkan call
            #define VK_ASSERT(f)																			                               \
            {																										                       \
                VkResult res = (f);																					                       \
                if (res != VK_SUCCESS)																				                       \
